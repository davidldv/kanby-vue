...
async function requireBoardForList(app, listId, userId) {
...
export async function cardsRoutes(app) {
    ...
    app.post('/cards/:cardId/move', async (req) => {
        ...
        const result = await app.prisma.$transaction(async (tx) => {
            ...
            if (!toList)
                throw app.httpErrors.notFound('Destination list not found');
            ...
            const destCards = fromList.id === toList.id
                ? sourceCards
            ...
            if (removed.index < 0)
                throw app.httpErrors.internalServerError('Card ordering mismatch');
            const toIndex = Math.max(0, Math.min(body.toIndex, fromList.id === toList.id ? removed.next.length : destIds.length));
            ...
            if (!after)
                throw app.httpErrors.internalServerError('Move failed');
            const event = await createActivityEvent(tx, {
