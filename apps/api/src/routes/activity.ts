...
export async function activityRoutes(app: FastifyInstance) {
  ...
  app.post('/activity/:eventId/undo', async (req, reply) => {
    ...
    if (!event) throw app.httpErrors.notFound('Activity event not found');
    ...
    const payload = safeJsonParse(event.rawPayloadJson) as any;

    const result = await app.prisma.$transaction(async (tx) => {
      const marked = await markEventUndone(tx, eventId);
      if (!marked.updated) {
        return { status: 'already-undone' as const };
      }

      // Apply reversal
      switch (event.type) {
        case 'CARD_CREATED': {
          const cardId = payload?.card?.id as string | undefined;
          if (!cardId) throw app.httpErrors.badRequest('Missing payload');
          await tx.card.delete({ where: { id: cardId } }).catch(() => undefined);
          break;
        }
        case 'CARD_UPDATED': {
          const before = payload?.before;
          if (!before?.id) throw app.httpErrors.badRequest('Missing payload');
        ...
        case 'CARD_DELETED': {
          ...
          if (!card?.id || !card?.listId) throw app.httpErrors.badRequest('Missing payload');
          const list = await tx.list.findUnique({
            where: { id: card.listId },
            select: { id: true, boardId: true },
          });
          if (!list) throw app.httpErrors.conflict('List no longer exists');
        ...
        case 'CARD_MOVED': {
          ...
          const fromListId = payload?.from?.listId as string | undefined;
          const fromIndex = payload?.from?.index as number | undefined;
          ...
          if (!card) throw app.httpErrors.conflict('Card no longer exists');

          const fromList = await tx.list.findUnique({
            where: { id: fromListId },
            select: { id: true },
          });
          if (!fromList) throw app.httpErrors.conflict('Original list no longer exists');
        ...
        case 'LIST_CREATED': {
          const listId = payload?.list?.id as string | undefined;
          if (!listId) throw app.httpErrors.badRequest('Missing payload');
        ...
        case 'LIST_DELETED': {
          ...
          if (!list?.id || !list?.boardId) throw app.httpErrors.badRequest('Missing payload');

          const existing = await tx.list.findUnique({
            where: { id: list.id },
            select: { id: true },
          });
          if (existing) throw app.httpErrors.conflict('List already exists');
      ...
      return { status: 'ok' as const, undoEvent };
    });
